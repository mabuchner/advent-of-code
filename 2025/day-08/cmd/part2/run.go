...
func process(boxes [][3]float64) int64 {
	...
	// Keeping making connections until all boxes are in the same cluster
	...
	for _, c := range connections {
		...
		// All boxes in the same cluster?
		if len(clusterToBoxes) == 1 {
			lastConnection = &c
			break
		}
	}

	// No more connections, but still more than one cluster?
	// (this shouldn't happen)
	if lastConnection == nil {
		return -1
	}

	// Multiply the x-coordinate of the last two connected boxes
	// to get the final result
	lastBoxI := boxes[lastConnection.boxI]
	lastBoxJ := boxes[lastConnection.boxJ]
	res := int64(lastBoxI[0]) * int64(lastBoxJ[0])
