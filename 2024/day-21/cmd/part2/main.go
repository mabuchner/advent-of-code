...
func run() error {
	// file, err := os.Open("./assets/input_small.txt") // 154115708116294
	file, err := os.Open("./assets/input.txt") // 252473394928452
	if err != nil {
	...
	for _, code := range codes {
		...
		humanInputLen := getHumanInputLen(code, 25)

		complexity += humanInputLen * num
	}
...
func getHumanInputLen(code string, robotDirpadCount int) int64 {
	// depth -> code -> len
	memo := make([]map[string]int64, robotDirpadCount+1)
	for i := range robotDirpadCount + 1 {
		memo[i] = make(map[string]int64, 100)
	}

	var dfs func(code string, depth int) int64
	dfs = func(code string, depth int) int64 {
		if l, ok := memo[depth][code]; ok {
			return l
		}

		if depth >= robotDirpadCount {
			return int64(len(code))
		}

		totalLen := int64(0)

		pos := dirpadButtonPositions['A']
		for i := range code {
			c := code[i]
			newPos := dirpadButtonPositions[c]
			input := dirpadInputs[[2][2]int{pos, newPos}]

			totalLen += dfs(input, depth+1)

			pos = newPos
		}

		memo[depth][code] = totalLen

		return totalLen
	}

	input := getNextInput(numpad, numpadButtonPositions, numpadInputs, code)
	totalLen := dfs(input, 0)
